...
public class SpawnUnit : MonoBehaviour
{
    ...
    /// </summary>
    public void AttemptToSpawnUnit(GameObject unit)
    {
        if (unit == null) return;
        int cost = unit.GetComponent<Stats>().Money;
        // 1. Kiểm tra tiền
        if (InGameMoney.Instance.SubMoney(cost)) // SubMoney đã bao gồm cả kiểm tra
        {
            // 2. Đủ tiền -> Spawn
            Debug.Log("Đã mua lính: " + unit.name);
        ...
        {
            // 3. Không đủ tiền
            Debug.Log("Không đủ tiền để mua " + unit.name);
